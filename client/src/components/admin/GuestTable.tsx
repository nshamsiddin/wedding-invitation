    ...
    <tr className="border-b border-white/5">
      {Array.from({ length: colSpan }).map((_, i) => (
        <td key={i} className="px-4 py-3">
          <div className="h-3 rounded bg-gray-800 animate-pulse" style={{ width: `${55 + (i * 17) % 40}%` }} />
        </td>
      ))}
    </tr>
  );
...
function CopyLinkButton({ invitation, baseUrl }: { invitation: AdminInvitation; baseUrl: string }) {
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-sans transition-colors focus:outline-none focus:ring-1 focus:ring-gold-400 bg-gray-800 hover:bg-gray-700 text-gray-300 hover:text-white"
    >
      ...
      <table className="w-full text-sm font-sans" aria-label="Guest list">
        <thead>
          <tr className="border-b border-white/10 bg-gray-900/80">
            {columns.map((col) => (
              <th key={col.key} scope="col" className="px-4 py-3 text-left font-medium text-gray-400 whitespace-nowrap">
                <button
            ...
            {events.map((ev) => (
              <th key={ev.id} scope="col" className="px-4 py-3 text-left font-medium text-gray-400 whitespace-nowrap">
            ...
            <tr>
              <td colSpan={totalColSpan} className="px-4 py-16 text-center">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-gray-600 text-xs">Try adjusting filters or add a guest manually</p>
                </div>
              </td>
            </tr>
          )}
            ...
            {!isLoading && sorted.map((guest) => (
              ...
              >
                <td className="px-4 py-3 text-white font-medium whitespace-nowrap max-w-[150px] truncate">
                  {guest.name}
                  {guest.phone && (
                    <p className="text-gray-600 text-xs font-normal mt-0.5">{guest.phone}</p>
                  )}
                </td>
                    ...
                    <td key={ev.id} className="px-4 py-3 min-w-[120px]">
                      {inv ? (
                        <div className="flex flex-col gap-1">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border self-start ${STATUS_COLORS[inv.status] ?? 'bg-gray-500/20 text-gray-400'}`}>
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              ...
                              title="Edit RSVP"
                              className="p-1 rounded text-gray-500 hover:text-gold-400 hover:bg-white/10 transition-colors focus:outline-none focus:ring-1 focus:ring-gold-400"
                            ...
                            <button
                              ...
                              className="p-1 rounded text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-colors focus:outline-none focus:ring-1 focus:ring-red-400"
