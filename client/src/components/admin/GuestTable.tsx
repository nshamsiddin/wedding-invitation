...
function SortIcon({ active, dir }: { active: boolean; dir: SortDir }) {
  return (
    <span className={`ml-1 inline-flex flex-col gap-px ${active ? 'text-blue-600' : 'text-gray-300'}`} aria-hidden="true">
    ...
    <tr className="border-b border-gray-100">
      {Array.from({ length: colSpan }).map((_, i) => (
        <td key={i} className="px-4 py-3">
          <div className="h-3 rounded bg-gray-100 animate-pulse" style={{ width: `${55 + (i * 17) % 40}%` }} />
        </td>
      ))}
    </tr>
  );
...
function CopyLinkButton({ invitation, baseUrl }: { invitation: AdminInvitation; baseUrl: string }) {
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-sans transition-colors focus:outline-none focus:ring-1 focus:ring-blue-500 bg-gray-100 hover:bg-gray-200 text-gray-600"
    >
      ...
      <table className="w-full text-sm font-sans" aria-label="Guest list">
        <thead>
          <tr className="border-b border-gray-200 bg-gray-50">
            {columns.map((col) => (
              <th key={col.key} scope="col" className="px-4 py-3 text-left font-medium text-gray-500 whitespace-nowrap">
                <button
                  onClick={() => handleSort(col.key)}
                  className="flex items-center gap-1 hover:text-gray-900 transition-colors focus:outline-none focus:text-blue-600"
            ...
            {events.map((ev) => (
              <th key={ev.id} scope="col" className="px-4 py-3 text-left font-medium text-gray-500 whitespace-nowrap">
            ...
            <tr>
              <td colSpan={totalColSpan} className="px-4 py-16 text-center">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-gray-600 font-medium text-sm">No guests found</p>
                  <p className="text-gray-400 text-xs">Try adjusting filters or add a guest manually</p>
                </div>
              </td>
            </tr>
          )}
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              ...
                              title="Edit RSVP"
                              className="p-1 rounded text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-colors focus:outline-none focus:ring-1 focus:ring-blue-500"
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
                    <button
                      ...
                      className="p-1.5 rounded-lg text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500"
