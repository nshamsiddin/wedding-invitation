...
const GLOBAL_CSS = `
  ...
  @keyframes gdn-petal-1 {
    0%   { transform: translate(0px, -10px) rotate(0deg);   opacity: 0; }
    10%  { opacity: 0.7; }
    90%  { opacity: 0.5; }
  ...
  @keyframes gdn-petal-2 {
    ...
    10%  { opacity: 0.65; }
    90%  { opacity: 0.4; }
    100% { transform: translate(55px, 110vh) rotate(-280deg); opacity: 0; }
  }
  @keyframes gdn-petal-3 {
    0%   { transform: translate(0px, -10px) rotate(0deg);   opacity: 0; }
    10%  { opacity: 0.75; }
    90%  { opacity: 0.45; }
    100% { transform: translate(-25px, 110vh) rotate(200deg); opacity: 0; }
  }
  @keyframes gdn-petal-4 {
    0%   { transform: translate(0px, -10px) rotate(0deg);   opacity: 0; }
    10%  { opacity: 0.6; }
    90%  { opacity: 0.35; }
    100% { transform: translate(35px, 110vh) rotate(-350deg); opacity: 0; }
  }
  @keyframes gdn-shimmer {
  ...
  .gdn-gold-shimmer {
    background: linear-gradient(90deg,
      ...
      rgba(184,146,74,1)   40%,
      rgba(220,186,110,1)  50%,
      rgba(184,146,74,1)   60%,
      rgba(184,146,74,0.5) 100%
    );
    background-size: 300% auto;
    -webkit-background-clip: text;
    background-clip: text;
    -webkit-text-fill-color: transparent;
    animation: gdn-shimmer 5s linear infinite;
  }

  @media (max-width: 480px) {
    .gdn-persistent-vine { display: none; }
    .gdn-side-vine        { display: none; }
  }

`;

// ─── Floating Petals (CSS-animated SVG petals) ───────────────────────────────
...
function generatePetals(count: number, colors: string[]): PetalDef[] {
...
function FloatingPetals({ petals }: { petals: PetalDef[] }) {
  return (
    <div
      aria-hidden="true"
...
// ─── Botanical vine corner SVGs ───────────────────────────────────────────────
function VineCornerTL({ inView }: { inView: boolean }) {
  return (
    <svg
...
function VineCornerBR({ inView }: { inView: boolean }) {
  return (
    <svg
      width="160" height="160" viewBox="0 0 160 160"
      style={{ position: 'absolute', bottom: 0, right: 0, transform: 'rotate(180deg)', width: 'min(160px, 25vw)', height: 'min(160px, 25vw)' }}
      aria-hidden="true"
    >
      <motion.path
        d="M8 8 C 20 30, 15 60, 30 90 C 45 120, 60 130, 80 140"
...
// ══════════════════════════════════════════════════════════════
// PRESET F — FIREFLY (3-layer depth + scroll/mouse parallax)
// ══════════════════════════════════════════════════════════════
interface Particle {
  x: number; y: number;
  vx: number; vy: number;
...
// Layer 0 = far/background, 1 = mid, 2 = near/foreground
const FF_LAYERS = [
...
function SideVinesFirefly({ wrapRef }: { wrapRef: RefObject<HTMLDivElement> }) {
  const canvasRef      = useRef<HTMLCanvasElement>(null);
  const mouseOffsetRef = useRef({ x: 0, y: 0 });
  const mouseRepelRef  = useRef({ x: -9999, y: -9999 });
  const prevScrollRef  = useRef(0);
  const scrollYRef     = useRef([0, 0, 0]); // parallax Y offsets per layer
  ...
  useEffect(() => {
    ...
    // Build particles for all three layers
    ...
    // Group by layer for efficient per-layer iteration
    const byLayer: Particle[][] = [[], [], []];
    for (const p of allParticles) byLayer[p.layer].push(p);

    const onMouseMove = (e: MouseEvent) => {
      // Repulsion origin (raw screen position)
      mouseRepelRef.current = { x: e.clientX, y: e.clientY };
      // Parallax offset (normalised from viewport centre)
    ...
    const draw = () => {
      ...
      // Update per-layer scroll parallax offsets
      ...
      // Draw back-to-front so near particles render on top
      for (let li = 0; li < 3; li++) {
        ...
        for (const p of pts) {
          ...
          // Near layer wraps; mid/far bounce
          if (L.wrap) {
        ...
        // Draw particles with depth-scaled glow
        for (const p of pts) {
        ...
        // Per-layer connection lines (drawn in parallax-shifted coordinates)
        ctx.save();
